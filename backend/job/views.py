...
from .serializers import CandidatesAppSerializer, JobSerializer
...
def getTopicStats(request, topic):
    ...
    stats = jobs.aggregate(
        total_jobs = Count('title'),                  # Here we get search positions
...
def applyToJob(request,pk):
    ...
    if job.lastDate < timezone.now():
        return Response( {'error': 'You can not apply this job. Date is over'}, status=status.HTTP_400_BAD_REQUEST)

    alreadyApplied = job.candidatesapplied_set.filter(user=user).exists()

    if alreadyApplied:
        return Response( {'error': 'You are already apply to this job. Date is over'}, status=status.HTTP_400_BAD_REQUEST)
...
def getCurrentUserAppliedJobs(request):
    ...
    serializer = CandidatesAppSerializer(jobs, many=True)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def getCerrentUserJobs(request):
...
def getCandidatesApplied(request,pk):
    ...
    serializer = CandidatesAppSerializer(candidates, many=True)
